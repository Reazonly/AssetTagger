        ...
        <form action="{{ route('master-data.asset-users.store') }}" method="POST">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required class="mt-1 block w-full border-2 border-gray-400 rounded-md shadow-sm py-2 px-3 @error('name') border-red-500 @enderror">
                    @error('nama') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="jabatan" class="block text-sm font-medium text-gray-700">Jabatan (Opsional)</label>
                    <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan') }}" class="mt-1 block w-full border-2 border-gray-400 rounded-md shadow-sm py-2 px-3 @error('name') border-red-500 @enderror">
                    @error('jabatan') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="departemen" class="block text-sm font-medium text-gray-700">Departemen (Opsional)</label>
                    <input type="text" name="departemen" id="departemen" value="{{ old('departemen') }}" class="mt-1 block w-full border-2 border-gray-400 rounded-md shadow-sm py-2 px-3 @error('name') border-red-500 @enderror">
