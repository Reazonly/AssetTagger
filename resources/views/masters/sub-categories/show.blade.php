...
<div class="bg-white rounded-xl shadow-lg p-6 md:p-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-200 pb-6 mb-6">
        <div>
            <a href="{{ route('master-data.sub-categories.index') }}" class="text-sm font-semibold text-gray-600 hover:text-gray-900 mb-2 inline-block">&larr; Kembali ke Daftar Kategori</a>
            <h1 class="text-3xl font-bold text-gray-900">Sub-Kategori: <span class="text-sky-600">{{ $category->name }}</span></h1>
        </div>
        <div class="mt-4 md:mt-0">
            <a href="{{ route('master-data.sub-categories.create', $category->id) }}" class="inline-flex items-center gap-2 bg-emerald-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-emerald-700"><span>Tambah Sub-Kategori</span></a>
        </div>
    </div>
