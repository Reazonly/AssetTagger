...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware(['role:admin'])->group(function() {
        ...
        // --- PERUBAHAN: Rute yang hanya bisa diakses oleh Super Admin (ID=1) ---
        Route::middleware(function ($request, $next) {
            if (auth()->id() !== 1) {
                return redirect()->route('users.index')->with('error', 'Anda tidak memiliki hak akses untuk tindakan ini.');
            }
            return $next($request);
        })->group(function () {
